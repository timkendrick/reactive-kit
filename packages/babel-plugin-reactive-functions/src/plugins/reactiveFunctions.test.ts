    ...
    const expected = template.program({ plugins: ['typescript'] } as TemplateBuilderOptions)
      .ast/* typescript */ `
        function foo(bar: string, baz: string) {
          return {
            [Symbol.for("@reactive-kit/symbols/hash")]: (_hash) => _hash(${inputHash}, bar, baz),
            [Symbol.iterator]: function* foo(): IteratorResult<any, string> {
              const x: number = 1;
              const y = x + (yield useFirst());
              const z = y + (yield useSecond());
              return \`Result: \${z}\`;
    ...
    const expected = template.program({ plugins: ['typescript'] } as TemplateBuilderOptions)
      .ast/* typescript */ `
        function foo(bar: string, baz: string) {
          return {
            [Symbol.for("@reactive-kit/symbols/hash")]: (_hash) => _hash(${inputHash}, bar, baz),
            [Symbol.iterator]: function* foo(): IteratorResult<any, CustomPromise<string> extends Promise<infer $T> ? $T : any> {
              const x: number = 1;
              const y = x + (yield useFirst());
              const z = y + (yield useSecond());
              return \`Result: \${z}\`;
